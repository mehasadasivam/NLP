...
#     # AS OF NOW: UNTIL 2016 ONLY *** TODO ****
# #     years = [str(year) for year in range(START_YEAR, END_YEAR + 1)]
#     years = [str(year) for year in range(START_YEAR, 2016 + 1)]
...
#     time_slices = article_count.sort_index()['No. of %s Articles' % THEME].values
    ...
    # SINCE WE'RE ONLY CREATING CORPUS UPTO 2016
    # Removing the last 4 years
    time_slices = time_slices[:-4]
    
    tic = time.time()
    # 5. Create the SeqLDA Model
    ldaseq = LdaSeqModel(corpus=corpus_all, id2word=dictionary_all, time_slice=time_slices, num_topics=5)   
    
    print('LDA Seq Model Created. Time Taken: %d seconds' % int(time.time() - tic))
    
    # 6. Save the model
    ldaseq.save(TEMP_PATH + '/%s/%s_LDASeqModel_yearly_5_Topics' % (THEME, THEME))
