        ...
        for art in data['Items']:
            ...
            if art['data']['language'] == 'en':
                ...
                if US_CODE in art['data']['subjects']:
                    # 3. Filter for articles about GDP only - but with similar words now!
                    if any(word in art['data']['body'].lower() for word in word_list):
                        all_gdp_articleids[fname].append(art['data']['id'])
                        all_gdp_article_texts[fname[-15:-9]].append(art['data']['body'])
        print(fname[-15:-9], len(all_gdp_articleids[fname]))
        del data

    with open(TEMP_PATH + '/GDP/GDP_Articles_v2_%s.json' % year, 'w') as f:
        json.dump(all_gdp_articleids, f)

    with open(TEMP_PATH + '/GDP/GDP_Article_Texts_v2_%s.json' % year, 'w') as f:
        json.dump(all_gdp_article_texts, f)
